...
# Register steps by updating the central registry
...
class LLMGenerateStep(BaseStep[LLMGenerateConfig]):
    ...
    async def execute(self, context: ContextProtocol) -> None:
        # Render templates for prompt, model, and output_key
        ...
        async def run_substep(idx: int, spec: Dict[str, Any]) -> None:
            ...
            try:
                ...
                if failure["exc"] is None:
                    ...
                    failure["idx"] = idx
                sub_logger.error(
                    "Substep %d failed: %s", idx, exc, exc_info=True
                )
                raise
