...
if len(key) <= 2:
return "_" _ len(key)
return f"{key[0]}{'_' _ (len(key) - 2)}{key[-1]}"
...
**all** = ["create_openai_responses_model"]
...
async def main_async() -> None: # Load environment variables from .env
load_dotenv()
...
except ValueError as ve: # Context or config parsing errors
...
=== File: recipe-executor/recipe_executor/steps/**init**.py ===
"""
...
**all** = ["ExecuteRecipeConfig", "ExecuteRecipeStep"]
...
rendered_list.append(\_render_config(item, context))
else:
...
"""Step to execute multiple sub-steps in parallel."""
    ...
    async def execute(self, context: ContextProtocol) -> None:
        ...
        for idx, spec in enumerate(substeps):
            ...
            if failure["exc"]:
                self.logger.debug(
                    "Fail-fast: aborting launch of remaining substeps at index %d", idx
                )
                break
...
Note: Does not account for content inside {% raw %}...{% endraw %} blocks.
"""
return "{{" in s or "{%" in s
...
**all** = ["json_object_to_pydantic_model"]
