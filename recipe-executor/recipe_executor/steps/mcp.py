...
from recipe_executor.steps.base import BaseStep, StepConfig, ContextProtocol
...
class MCPStep(BaseStep[MCPConfig]):  # type: ignore
    ...
    def __init__(self, logger: logging.Logger, config: Dict[str, Any]) -> None:
        ...
        validated: MCPConfig = MCPConfig.model_validate(config)  # type: ignore
    ...
    async def execute(self, context: ContextProtocol) -> None:
        ...
        if "command" in server_conf:
            ...
            if server_conf.get("env") is not None:
                config_env = {}
                for env_k, env_v in server_conf.get("env", {}).items():  # type: ignore
            ...
            if server_conf.get("headers") is not None:
                headers_conf = {}
                for hk, hv in server_conf.get("headers", {}).items():  # type: ignore
            ...
            async with client_cm as (read_stream, write_stream):
                async with ClientSession(read_stream, write_stream) as session:
                    ...
                    try:
                        ...
                        raise ValueError(f"Tool invocation failed for '{tool_name}' on {service_desc}: {exc}") from exc
        except ValueError:
            # Propagate invocation errors
            raise
        except Exception as exc:
            raise ValueError(f"Failed to call tool '{tool_name}' on {service_desc}: {exc}") from exc
