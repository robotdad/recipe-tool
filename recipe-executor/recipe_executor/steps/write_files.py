...
class WriteFilesStep(BaseStep[WriteFilesConfig]):
    ...
    It supports:
      - Direct `files` entries with `path`/`content` or via `path_key`/`content_key`.
      - Context-based `files_key` pointing to FileSpec, dict, or list of such.
      - Template rendering for all file paths.
      - Automatic JSON serialization for dicts/lists.
      - Debug/info logging of file operations.
    """
    ...
    async def execute(self, context: ContextProtocol) -> None:
        ...
        if self.config.files is not None:
            for entry in self.config.files:
                ...
                path_str: str = str(raw_path)
                path: str = render_template(path_str, context)

                # Resolve content
                if "content" in entry:
                    raw_content: Any = entry["content"]
        ...
        # 2. Use files from context via 'files_key'
            ...
            key: str = self.config.files_key
            if key not in context:
                raise KeyError(f"Files key '{key}' not found in context.")
            raw: Any = context[key]

            # Normalize to a list of specs
            ...
            for item in items:
                # Extract path/content from FileSpec or dict
                if isinstance(item, FileSpec):
                    raw_path, raw_content = item.path, item.content
        ...
        for entry in files_to_write:
            ...
            # Determine final file system path
            if root:
                combined = os.path.join(root, rel_path)
            else:
                combined = rel_path
            final_path: str = os.path.normpath(combined)
            ...
            try:
                ...
                # Serialize content if it's a dict or list
                if isinstance(content, (dict, list)):
                    try:
                        text: str = json.dumps(content, ensure_ascii=False, indent=2)
                    except Exception as err:
                        raise ValueError(f"Failed to serialize content for '{final_path}': {err}")
                else:
                    # Convert None to empty string, others to str
                    if content is None:
                        text = ""
