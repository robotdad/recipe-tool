...
def create_resource_editor() -> Dict[str, gr.components.Component]:
    ...
    with gr.Column(visible=False) as container:
        ...
        path = gr.Textbox(
            label="File Path / URL",
            placeholder="/path/to/file.txt or https://example.com/data"
        )
    ...
    with gr.Blocks(title="Document Generator", theme="soft", css=custom_css) as app:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2, min_width=300):
                ...
                with gr.Group():
                    ...
                    gr.Markdown("*Note: Changing resource keys may require re-selecting sections that reference them*", 
        ...
        def handle_selection(selected_id, selected_type, current_state):
            ...
            return [
                ...
                res_key, res_desc, res_path, res_merge,
                sec_title, sec_mode, sec_prompt,
                gr.update(
        ...
        def auto_save_resource_field(field_name, value, current_state):
            ...
            if field_name == "key":
                return (
                    ...
                    json_str, validation_msg, generate_btn_update
                )
        ...
        resource_remove_btn.click(
            ...
            outputs=[state, resource_radio, section_radio, empty_state,
                    resource_editor["container"], section_editor["container"],
                    json_preview, validation_message, generate_btn]
        )
        ...
        resource_editor["file"].change(
            lambda v, s: auto_save_resource_field("path", v, s) if v else (s, gr.update(), gr.update(), gr.update(), gr.update(), gr.update()),
        ...
        app.load(
            ...
            inputs=[state],
            outputs=[json_preview, validation_message, generate_btn]
        )
