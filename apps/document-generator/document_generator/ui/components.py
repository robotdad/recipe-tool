"""
Reusable UI components for the Document Generator editor.
"""
import gradio as gr

def resource_entry(key: str = "", description: str = "", path: str = "", merge_mode: str = "concat"):
    """
    Return Gradio components for a single resource entry row.
    """
    # TODO: implement UI components (Textbox, File, Dropdown)
    return []

def section_entry(section=None):
    """
    Return Gradio components for a single section entry (with nested support).
    """
    # TODO: implement UI components (Accordion, Textbox, TextArea, Dropdown, Toggle)
    return []